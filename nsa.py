...
def scanXhicoS8():
    if debug: print("Scanning for bluetooth")
    ...
    while True:
        ...
        if not line: break
            ...
            if "Host is down" in str(line): return False
            else: return True
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            if debug: print("isHere - " + str(isHere))
            if debug: print("isRunning - " + str(isRunning))
            
            if isHere and isRunning: stopRun()
            elif not isHere and not isRunning: startRun()
